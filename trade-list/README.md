...
圖的最底部固定有一條 `● pogokit.com/t │ 副標`(排在同一行),由 `drawBrand()` 畫。
整條佔 `BRAND_H` = 62px:**這是加在圖的總高上的**,清單只有幾隻時那塊空白會比卡片還顯眼,
所以寧可擠一點 —— 上下堆疊的版本光文字就要 83px,排成一行後面板底到圖底從 192px 降到 80px。
字級不是逐項各縮各的,而是先用基準字級量一次、再用**一個縮放係數**把整組等比例縮到塞得下
(`k`),不然圓點、間距、副標的比例會跑掉;窄圖(每排 3 隻)與德文副標都會走到這條路。

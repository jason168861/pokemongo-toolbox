...
圖的最底部固定有一條 `● pogokit.com/t` + 下面一行副標,由 `drawBrand()` 畫。
整條佔 `BRAND_H` = 92px:**這是加在圖的總高上的**,清單只有幾隻時那塊空白會比卡片還顯眼,
所以留白抓得比一般頁尾緊(面板底到圖底 110px)。
字級不是逐項各縮各的,而是先用基準字級量一次、再用**一個縮放係數**把整組等比例縮到塞得下
(`k`),不然圓點、間距、副標的比例會跑掉。縮放看的是**兩行裡比較寬的那一行** ——
德文副標比網址還長,只量網址會爆出圖外。
